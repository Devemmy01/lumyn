...
export function AcademyInstallPrompt({
  ...
  return (
    ...
    >
      <section
        className="relative flex max-h-[92svh] w-full max-w-[540px] flex-col overflow-hidden rounded-t-[1.75rem] border border-white/10 bg-[#101118] text-white shadow-[0_35px_120px_rgba(0,0,0,0.5)] sm:max-h-[90vh] sm:rounded-[2.3rem]"
      ...
      >
        ...
        <div className="academy-scrollbar min-h-0 flex-1 overflow-y-auto overflow-x-hidden">
        <div className="relative p-5 pb-4 sm:p-6 sm:pb-4">
          <div className="flex items-center gap-3 pr-10">
            <div className="grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-xl border border-white/10 bg-gradient-to-br from-[#9185ff] to-[#6655dc] shadow-[0_18px_45px_rgba(124,108,246,0.3)] sm:h-12 sm:w-12">
              <AstraMascot expression="happy" className="h-12 w-12 translate-y-1 sm:h-14 sm:w-14" />
            </div>
            <div className="min-w-0">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#b9b1ff]">
                Your academy, one tap away
              </p>
              <h2 id="academy-install-title" className="mt-1 text-lg font-semibold tracking-[-0.035em] sm:text-xl">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {[
              ...
              <span key={label} className="inline-flex items-center gap-1.5 rounded-full border border-white/[0.08] bg-white/[0.045] px-3 py-1.5 text-[11px] font-bold text-white/65">
          ...
          {ios && (
            <div className="mt-4 rounded-2xl border border-[#21e0aa]/20 bg-[#21e0aa]/[0.07] p-3.5">
              <p className="text-xs font-bold text-[#8ff5d5]">Install on iPhone or iPad</p>
              <ol className="mt-2 space-y-1.5 text-xs leading-5 text-white/62">
              ...
              {legacyIOS && (
                <p className="mt-2 border-t border-[#21e0aa]/15 pt-2 text-[10px] leading-4 text-white/45">
          ...
          {manual && (
            <div className="mt-4 rounded-2xl border border-[#7c6cf6]/20 bg-[#7c6cf6]/[0.08] p-3.5 text-xs leading-5 text-white/62">
        ...
        <div className="relative grid shrink-0 gap-2 border-t border-white/[0.08] bg-black/15 p-4 pb-[max(1rem,env(safe-area-inset-bottom))] sm:grid-cols-[1fr_auto] sm:p-4">
          {status === "available" ? (
            <button
              type="button"
              onClick={onInstall}
              className="inline-flex h-11 items-center justify-center gap-2 rounded-2xl bg-[#7c6cf6] px-6 text-sm font-black text-white shadow-[0_14px_32px_rgba(124,108,246,0.28)] transition hover:-translate-y-0.5 hover:bg-[#897af8]"
            >
              <InstallIcon /> Install free
            </button>
          ) : (
            <button type="button" onClick={onClose} className="h-11 rounded-2xl bg-[#7c6cf6] px-6 text-sm font-black text-white transition hover:bg-[#897af8]">
              Got it
            </button>
          )}
          <button type="button" onClick={onDismiss} className="h-11 rounded-2xl px-5 text-xs font-bold text-white/42 transition hover:bg-white/[0.05] hover:text-white/70">
            Maybe later
          </button>
        </div>
      </section>
