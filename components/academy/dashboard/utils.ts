...
export function getCertificateVerificationOrigin(origin = "") {
  const configuredOrigin = process.env.NEXT_PUBLIC_SITE_URL?.replace(/\/$/, "");
  if (configuredOrigin) return configuredOrigin;
  const normalizedOrigin = origin.replace(/\/$/, "");
  if (normalizedOrigin.includes("lumynhq.studio")) return normalizedOrigin;
  return "https://lumynhq.studio";
}
...
export async function openCertificate(studentName: string, course: DashboardCourse) {
  ...
  const publicOrigin = getCertificateVerificationOrigin(origin);
  ...
  certificateWindow.document.write(`<!doctype html><html><head><title>${escape(course.certificate.certificateId)}</title><style>
    *{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact}
    ...
    .content{position:relative;z-index:3;display:grid;height:100%;grid-template-columns:minmax(0,1fr) 274px;gap:34px;padding:56px 66px 46px}
    ...
    .copy{margin-top:24px;max-width:660px;color:#d8d2eb;font-size:14px;line-height:1.65}
    ...
    .side{position:relative;border-left:1px solid rgba(255,255,255,.16);padding-left:28px}
    ...
    .verify{width:216px;margin-top:124px;padding:15px;border:1px solid rgba(255,255,255,.18);border-radius:14px;background:rgba(13,17,34,.44);backdrop-filter:blur(6px)}
    .verifyLabel{color:#9d96b5;font-size:10px;font-weight:900;letter-spacing:.22em;text-transform:uppercase}
    .verifyTitle{margin-top:9px;color:#f7f4ff;font-size:13px;font-weight:900;letter-spacing:.05em;text-transform:uppercase}
    .verifyId{margin-top:8px;color:#b9b1ff;font-family:"Courier New",monospace;font-size:11px;font-weight:700;word-break:break-all}
    .verifyLink{display:block;margin-top:11px;padding:0;color:#d9d4ec;font-family:"Courier New",monospace;font-size:7.5px;line-height:1.45;word-break:break-all;text-decoration:none}
    .verifyLink:hover{background:rgba(95,124,255,.14)}
    .qrBox{display:flex;width:124px;margin:13px auto 0;padding:8px;border-radius:8px;background:#fff}
    .qrBox svg{display:block;width:108px;height:108px}
